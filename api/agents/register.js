...
import {
  q, send, readBody, PROTOCOL_VERSION, issueToken, sha256hex, newId, utcnow,
  validatePublicName, publicParticipant,
} from '../_db.js';
...
export default async function handler(req, res) {
  ...
  const checkedName = validatePublicName(body.display_name || body.name);
  if (checkedName.error) return send(res, 400, { error: checkedName.error });
  ...
  const publicIdentity = publicParticipant(name);
  return send(res, 200, {
    agent_id: id,
    agent_token: token,
    protocol_version: PROTOCOL_VERSION,
    public_name: publicIdentity.name,
    public_ref: publicIdentity.ref,
  });
}

...
Memory = the agent's own session. By default it is kept alive across one game (the `session_agent`
model, not the `file_memory_agent` fresh-read model); set ARENA_AGENT_RESET_BETWEEN_GAMES=0 to keep
one session across the whole run instead. Either way a new run gets a fresh state key. The mechanics:
  ...
  - `act` flushes that buffer as the turn's single message and sends it into the RESUMED session for
    ...
    carries its own reasoning, prior turns, and any scratch files across the configured scope — it
    is never handed a fresh, amnesiac session mid-scope.
...
def opening_prompt(history_text: str, turn) -> str:
    """First message of a state-key session: system framing + everything observed before this turn."""
    return (
        f"{SYSTEM}\n\n{_WORKSPACE_NOTE}\n\n"
...
class SessionCodingHarness:
    ...
    Default scope is per-game; ARENA_AGENT_RESET_BETWEEN_GAMES=0 switches to one session per run.
    ...
    def on_event(self, event) -> None:
        ...
        if key is None:
            return  # run-level event, not part of any game/run-scoped memory

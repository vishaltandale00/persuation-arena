...
class SeatState:
    ...
    def apply(self, event: Any) -> None:
        ...
        if etype == "game_setup":
            ...
            self.public.append(f"{self._name(p.get('actor_seat'))}: {p.get('text', '')}")
        elif etype == "pass":
            self.public.append(f"{self._name(p.get('actor_seat'))} passes.")
    ...
    def _name(self, seat: Any) -> str:
        try:
            return self.roster.get(int(seat), f"seat {seat}")
    ...
    def render_context(self, *, phase: str = "context", action_kind: str = "onuw.context") -> str:
        ...
        lines = [
            f"You are {self.roster.get(self.seat)}, seat {self.seat}, in a {self.n}-player One Night Ultimate Werewolf game.",
